	...
	class Konsumpay_Auth {
		...
		public function login($username, $password, $user_type="users")
		{
			...
			switch($user_type) {
				...
				case "owner" :
					$verifyInfo = $this->_CI->konsumpay_auth_model->verify_owner($username, $this->encrypt_password($password));
				break;
			}
		...
		/**
		 * Verify Company Name
		 * @param unknown_type $comp_name
		 */
		public function verify_company_name($comp_name){
			$verify_comp_name = $this->_CI->konsumpay_auth_model->verify_comp_name($comp_name);
			if($verify_comp_name){
				return TRUE;
			}else{
				return FALSE;
