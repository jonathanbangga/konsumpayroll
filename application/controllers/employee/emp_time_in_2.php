	...
	class Emp_time_in extends CI_Controller {
		...
		public function index() {
			...
			if($this->input->is_ajax_request()) {
				...
				if($this->input->post('process_time_out')){
					...
					if($time_in_is_empty){
						...
						if($check_current_time_to_login){
							echo json_encode(array("success"=>3,"msg"=>"- Please try to login after {$this->min_log} minutes"));
							return false;
						}
						...
						if($get_datetime_in_today != FALSE){
							...
							}elseif($current_time_in != "00:00:00" && $current_lunch_out != "" && $current_lunch_in != "" && $current_time_out == "00:00:00"){
				...
				if($this->input->post('change_log')){
					...
					if($get_timein_info){
						...
						$lunch_out = ($get_timein_info->lunch_out=="00:00:00") ? $get_timein_info->lunch_out : date("g:i:A",strtotime($get_timein_info->lunch_out));
						$lunch_in = ($get_timein_info->lunch_in=="00:00:00") ? $get_timein_info->lunch_in : date("g:i:A",strtotime($get_timein_info->lunch_in));
						$time_out = ($get_timein_info->time_out=="00:00:00") ? $get_timein_info->time_out : date("g:i:A",strtotime($get_timein_info->time_out));
			...
			if($this->input->post('update_log')){
				...
				$time_in = date("H:i:s",strtotime($time_in_hr.":".$time_in_min." ".$time_in_ampm));
				...
				if ($this->form_validation->run()==true){
					$update_employee_time_log = $this->employee->update_employee_time_log(
						$this->company_id, $this->emp_id, $employee_timein, $time_in, $lunch_out, $lunch_in, $time_out, $reason
					);
