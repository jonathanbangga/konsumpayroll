...
class Payroll_group extends CI_Controller {
	...
	public function index(){
		...
		// data
		$data['pg_sql'] = $this->payroll_group_model->get_payroll_group();
		$this->layout->view('pages/payroll_setup/payroll_group_view',$data);
	}
	
	public function ajax_add_payroll_group(){
		...
		foreach($payroll_group as $index=>$val){
			$this->payroll_group_model->add_payroll_group($val,$period_type[$index],$pay_rate_type[$index]);
		}
	}
	
	public function ajax_delete_payroll_group(){
		$pg_id = $this->input->post('pg_id');
		$this->payroll_group_model->delete_payroll_group($pg_id);
	}
	
	public function ajax_update_payroll_group(){
		$pg_id = $this->input->post('pg_id');
		$payroll_group = $this->input->post('payroll_group');
		$period_type = $this->input->post('period_type');
		$pay_rate_type = $this->input->post('pay_rate_type');
		$this->payroll_group_model->update_payroll_group($pg_id,$payroll_group,$period_type,$pay_rate_type);
	}
